...
use mpsgraph::{
    CompilationDescriptor, DataType, Executable, ExecutableExecutionDescriptor,
    Graph, GraphCallOps, Shape, ShapedType, TensorData,
};
use ndarray::Array2;
...
fn main() {
    ...
    let max_diff = difference.iter().fold(0.0f32, |max_val, &val| max_val.max(val.abs()));
