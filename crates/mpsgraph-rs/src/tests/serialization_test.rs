#[test]
fn test_graph_compile_serialize_deserialize_run() {
    use crate::{
    ...
    // --- 1. Graph Definition ---
    let graph = crate::Graph::new(); // Use crate::Graph
    let shape_dims = [2, 2];
    ...
    let mps_device = Device::new(); // Use new() instead of custom_default()
    ...
    temp_file_path.push("test_graph.mpsgraphpkg");
    ...
    let _deserialized_executable = deserialized_executable_opt.unwrap(); // Keep variable to avoid drop
    println!("Deserialization successful.");

    // --- 5. Execution (Requires additions to Executable bindings) ---

    // **Placeholder:** Need methods on `Executable` to get feed/target tensors
    let feed_tensors = _deserialized_executable
        .feed_tensors()
        .expect("Failed to get feed tensors");
    let target_tensors = _deserialized_executable
        .target_tensors()
        .expect("Failed to get target tensors");
    ...
    // Note: Relying on order here. A more robust test might check tensor names if available.
    let input_tensor_a = &feed_tensors[0];
    let input_tensor_b = &feed_tensors[1];
    let output_tensor_c = &target_tensors[0];
    ...
    let inputs_vec = vec![&a_tensor_data, &b_tensor_data]; // Order matters!
    let results_vec = vec![&result_tensor_data];

    // Prepare for execution
    let command_queue = metal_device.new_command_queue();
    let command_buffer = crate::CommandBuffer::from_command_queue(&command_queue);
    let exec_desc = ExecutableExecutionDescriptor::new(); // Use ExecutableExecutionDescriptor
    exec_desc.set_wait_until_completed(true); // Run synchronously for simplicity

    println!("Executing deserialized graph...");
    // Call the newly added execution method
    let execution_results = _deserialized_executable.encode_to_command_buffer(
        &command_buffer,
        &inputs_vec,
        Some(&results_vec),
        Some(&exec_desc),
    );
    // We are providing results buffer, so the returned array might be empty or nil, depends on API detail.
    // assert!(execution_results.is_some(), "Execution returned None");
    ...
    let _ = fs::remove_file(temp_file_path); // Ignore error if file doesn't exist
}
