...
fn test_data_type_attribute_value() {
    ...
    assert!(
        shaped_type_opt.is_some(),
        "Expected to retrieve a shaped type from attribute"
    );

    // Test creation using a standard constructor instead of CustomDefault
    let default_attr: Retained<DataTypeAttributeValue> = DataTypeAttributeValue::float32();
