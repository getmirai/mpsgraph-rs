...
use mpsgraph::{
    DataType, Graph, Tensor, Shape, ShapeExtensions,
...
mod tests {
    ...
    fn test_tensor_addition() {
        ...
        let a = Tensor::new(graph.placeholder(&shape, DataType::Float32, None));
        let b = Tensor::new(graph.placeholder(&shape, DataType::Float32, None));

        let sum = &a + &b;

        assert_eq\!(sum.0.data_type(), DataType::Float32);
